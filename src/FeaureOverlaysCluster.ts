...
export default class FeaureOverlaysCluster extends EventEmitter {
        ...
        if (overlayers.type === 'group') {
            ...
            if ((overlayers as Overlayerbase).overlaytype.toLowerCase() === 'marker') {
                ...
                const clientoperateLayer = new FeatureLayer({
                    id: this.displayedLayerid,
                    title: "Overlayer-marker-" + this.displayedLayerid,
                    visible: (overlayers as Overlayerbase).visible,
                    objectIdField: 'objectId',
                    geometryType: 'point',
                    renderer: markrenderer,
                    screenSizePerspectiveEnabled: true,
                    maxScale: (overlayers as Overlayerbase).maxScale,
                    minScale: (overlayers as Overlayerbase).minScale,
                    popupEnabled: false,
                    popupTemplate: false,
    ...
    public async addfeature<T extends any[] = any[]>(overlayers: Overlayerbase | Overlayerbase[]): Promise<T> {
        // tslint:disable-next-line:variable-name
        const [FeatureLayer, LabelClass, Graphic, Point, esriCircle, PictureMarkerSymbol,
            // tslint:disable-next-line:variable-name
            ArcGISPolyline, ArcGISPolygon] = await load(['esri/layers/FeatureLayer', 'esri/layers/support/LabelClass', 'esri/Graphic', 'esri/geometry/Point',
                'esri/geometry/Circle', 'esri/symbols/PictureMarkerSymbol', "esri/geometry/Polyline", "esri/geometry/Polygon"]);
    ...
    public async updatefeature<T extends any[] = any[]>(overlayers: Overlayerbase | Overlayerbase[] | OverlayGroup)
    : Promise<T>  {
        // tslint:disable-next-line:variable-name
        const [FeatureLayer, LabelClass, Graphic, Point, esriCircle, PictureMarkerSymbol,
            // tslint:disable-next-line:variable-name
