define(["dojo/_base/declare", "dojo/_base/lang", "esri/geometry/Point", "esri/geometry/SpatialReference"],
	function (declare, lang, n, SpatialReference) {
		return declare("EchartsglLayer", null, {
			...
			echartlayerid: null,
			visible: true,
			constructor: function (view, option, echartlayerid) {
				echarts.registerCoordinateSystem('arcgis', this.getE3CoordinateSystem(view));
				this.init(view, option, echartlayerid);

			},
			init: function (view, option, echartlayerid) {
				this.echartlayerid = echartlayerid;
			...
			getE3CoordinateSystem: function (map) {
				...
				CoordSystem.prototype.dataToPoint = function dataToPoint(data) {
					var sp;
					if (map.spatialReference.wkt === null) {
						sp = new SpatialReference(102100)
					} else {
						sp = new SpatialReference({
